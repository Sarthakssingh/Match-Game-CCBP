...
import Gameover from './Component/Gameover/index'
import Gamepage from './Component/Gamepage/index'
...
class App extends Component {
  ...
  thumbnails = category => {
    const list = imagesList.filter(eachValue => eachValue.category === category)
    return list
  }
  ...
  render() {
    const {lostGame, category, score, time, imgUrl} = this.state
    const thumbnailList = this.thumbnails(category)
    return (
            ...
            <div className="sub-content">
              ...
              <ul className="thumbnail-images">
                {thumbnailList.map(eachItem => (
                  <Gamepage
                    key={eachItem.id}
                    thumbnailUrl={eachItem.thumbnailUrl}
